...
const Register = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!/^[a-zA-Z_ ]+$/.test(username)) {
      setError({
        ...error,
        username: 'Username must contain only letters, spaces, or underscores',
      });
      return;
    }
    setError({ ...error, username: '' });
...
const Register = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        // Simulating API Call
>>>>>>> fe3169fafda8fb375c7a55b38b4ab5ca910a61b8
        ...
        <div className="w-[400px] backdrop-blur-sm p-8 rounded-xl shadow-lg border border-white/20">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="relative">
              <label htmlFor="profilePic" className="block text-sm font-medium text-white mb-2">
              ...
              />
              {error.profilepic && <p className="text-red-400 text-sm mt-1">{error.profilepic}</p>}
              {profilePic && (
                <img
                  src={profilePic}
                  alt="Preview"
            ...
            <div className="relative">
              <label
                htmlFor="email"
                className={`absolute left-3 top-3 transition-all text-gray-300 text-sm ${
                  registerForm.email ? 'top-[-10px] text-xs text-white bg-gray-600 px-1' : 'top-3'
                }`}
              >
                Enter email
              </label>
