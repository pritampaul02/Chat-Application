 ...
 console.log("user ========> " , user);
  ...
  async loginUser(email, password) {
    ...
    if (user.isTwoStepAuth) {
      const otp = genarate6DigitOtp()
      user.otp = otp
      user.otpExpiary = Date.now() + 5 * 60 * 1000
      await user.save()
  ...
  async sendFriendRequest(userId, body) {
    ...
    io.to(reciverSocketId).emit("friendRequest", { senderId: userId, senderName: sender.name });

    const senderSocketId = await getSocketId(userId);
    console.log("sender socket id " , senderSocketId);
    
    io.to(senderSocketId).emit("sendFriendRequest", { senderId: receiverId, senderName: receiver.name });
